...
2017.04.26     - v3.1
                 it was saving the center of the field as the position of
                 the object, but it was not always the truth.
...
def extract_stamps(framelocalbz2, thisframeIDs):
    ...
    for ii in thisframeIDs:
        ...
        #object position in pixel
        x0_obj, y0_obj = stamp['wcs'].wcs2pix(ras[ii], decs[ii])
        ...
        # if NOT,
        # 1. recenter obj in the image by redefining SZdeg
        # 2. put a FLAG in the header
        ...
        if stampNpix < 0.9*teoNpix :
            ...
            print ('incomplete:', fraction,)
      
            print ('reshaping stamp')
            M,N = stamp['data'].shape
            # choose the smaller distance from object to image border and convert in degrees
            newSZdeg =    np.min((x0_ref, y0_ref, M - y0_ref, N - x0_ref )) * stamp['wcs'].getPixelSizeDeg()
            stamp = astImages.clipImageSectionWCS(frame,framewcs, ras[ii] , decs[ii], float(newSZdeg))
        ...
        stamp['wcs'].header.update({'OBJID10' : (objids[ii], 'MFMTK: SDSS DR10 Object ID') })
        stamp['wcs'].header.update({'OBJXPIX' : (x0_obj,     'MFMTK: Object pixel X location') })
        stamp['wcs'].header.update({'OBJYPIX' : (y0_obj,     'MFMTK: Object pixel Y location') })
        stamp['wcs'].header.update({'ASECPIX' : (stamp['wcs'].getPixelSizeDeg()*60*60, 'MFMTK: Pixel size in arcsec') })
